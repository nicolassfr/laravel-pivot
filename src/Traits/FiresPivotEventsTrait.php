...
namespace Nicolassfr\Laravel\Pivot\Traits;
...
trait FiresPivotEventsTrait
{
     ...
     * @return mixed
     */
    public function sync($ids, $touch = true)
    {
        list($idsOnly, $idsAttributes) = $this->getIdsWithAttributes($ids, []);

        $this->parent->fireModelEvent('pivotSyncing', true, $this->getRelationName(), $idsOnly, $idsAttributes);
        $parentResult = parent::sync($ids, $touch);
        $this->parent->fireModelEvent('pivotSynced', false, $this->getRelationName(), $idsOnly, $idsAttributes);
